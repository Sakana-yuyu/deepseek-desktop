...
本机已有兼容 Node 时跳过运行时压缩包下载；捆绑源码树还没有 `node_modules` 时，仍会执行 `pnpm install`。之后的启动只要仍有该树和已记录的二进制，就会跳过释放和安装。`~/.dsh` 已存在时，桌面端与 CLI 共用一个主目录，包括会话和密钥。overlay 插件写入选定主目录。主机 Node 后来消失时，下一次启动的跳过失败并回退到扫描，再回退到私有运行时。已选定的二进制随后由[桌面端 PATH 桥接](2026-08-14-desktop-path-bridge.md)暴露到 PATH。
