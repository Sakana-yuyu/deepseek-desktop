...
**Release builds check and install updates after the main window opens.** The official Tauri updater validates the stable manifest and artifact signature, installs a newer semantic version, and restarts the application. A failed update check or download is logged and does not hold the splash. Development builds skip network update checks. A ready runtime manifest skips the host toolchain scan and compares Node by file size instead of hashing `node.exe`.
